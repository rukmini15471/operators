...
def calculate_extras():
    ...
    total_extras = (
        no_balls +  # Each no-ball adds 1 run
        wides +     # Each wide adds 1 run
        byes +      # Each bye adds 1 run
        leg_byes +  # Each leg-bye adds 1 run
        (penalties * 5)  # Each penalty adds 5 runs
    )
    print(total_extras)
calculate_extras()
